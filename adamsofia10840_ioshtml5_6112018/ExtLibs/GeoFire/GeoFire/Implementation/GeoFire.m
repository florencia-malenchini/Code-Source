               ...
               withBlock:(GFCompletionBlock)block
{
    ...
    if (location != nil) {
        ...
        value = @{ @"posicao": @{@"latitude": lat, @"longitude": lng}, @"g": geoHash };
        priority = geoHash;
    } else {
        value = nil;
        priority = nil;
...
{
    if ([value isKindOfClass:[NSDictionary class]] && [value objectForKey:@"geofire"] != nil) {
        id locObj = [value valueForKeyPath:@"geofire.posicao"];
        if ([locObj isKindOfClass:[NSDictionary class]] && [locObj count] == 2) {
            id latNum = [locObj valueForKey:@"latitude"];
            id lngNum = [locObj valueForKey:@"longitude"];

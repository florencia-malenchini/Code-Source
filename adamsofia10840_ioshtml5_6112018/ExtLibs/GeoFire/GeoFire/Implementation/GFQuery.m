...
{
    return [[[self.geoFire.firebaseRef queryOrderedByChild:@"geofire/g"] queryStartingAtValue:query.startValue]
            queryEndingAtValue:query.endValue];
}

- (void)updateLocationInfo:(FIRDataSnapshot *) snapshot forLocation:(CLLocation *)location forKey:(NSString *)key
{
    ...
    if ((isNew || !wasInQuery) && info.isInQuery) {
        [self.keyEnteredObservers enumerateKeysAndObjectsUsingBlock:^(id observerKey,
            ...
            dispatch_async(self.geoFire.callbackQueue, ^{
                block(snapshot, key, info.location);
            });
        }];
...
- (void)childAdded:(FIRDataSnapshot *)snapshot
{
    @synchronized(self) {
        ...
        if (location != nil) {
            [self updateLocationInfo:snapshot forLocation:location forKey:snapshot.key];
        } else {
            // TODO: error?
        }
    }
...
- (void)updateQueries
{
    ...
    [self.locationInfos enumerateKeysAndObjectsUsingBlock:^(id key, GFQueryLocationInfo *info, BOOL *stop) {
        [self updateLocationInfo:info.snapshot forLocation:info.location forKey:key];
    }];
                ...
                dispatch_async(self.geoFire.callbackQueue, ^{
                    @synchronized(self) {
                        [self.locationInfos enumerateKeysAndObjectsUsingBlock:^(NSString *key,
                                                                                ...
                                                                                BOOL *stop) {
                            if (info.isInQuery) {
                                block(info.snapshot, key, info.location);
                            }
                        }];
                    };
                });
